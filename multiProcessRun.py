...
def clusterAnalysis(name, radius, sizeROI, rInner, addToROI, TSMin, sqrtTsThreshold, minSeparation, maxIter, dmMin, TSm1Min, logeBins, debug) :
...
for i in [0, 10] :
	for j in range(len(clusters[i]) - 3) :
		...
		processes.append(Process(target=clusterAnalysis, args=(name, radius, sizeROI, rInner, addToROI, TSMin, sqrtTsThreshold, minSeparation, maxIter, dmMin, TSm1Min, logeBins, debug)))	
